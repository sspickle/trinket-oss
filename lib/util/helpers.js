...
// Gate course creation: approved instructors and admins only
internals.isInstructor = function(user) {
  var instructorAuth = require('./instructorAuth');
  if (!user) throw Boom.unauthorized();
  if (instructorAuth.isAdminEmail(user.email)) return defaultNextResult;
  if (user.isInstructor) return defaultNextResult;
  throw Boom.forbidden('Only approved instructors can create courses.');
}
...
module.exports.register = function(server) {
  ...
  server.method('isInstructor',         internals.isInstructor);
