...
internals.isApproved = function(user) {
  ...
  if (instructorAuth.isAdminEmail(user.email)) return defaultNextResult;
...
// controllers without throwing. Restricts course create/copy to admins (via
// ADMIN_EMAILS) and approved instructors (via instructormi.org). Students,
// including those with elevated per-course roles, cannot create or copy.
internals.userCanCreateCourse = function(user) {
  if (!user) return false;
  var instructorAuth = require('./instructorAuth');
  if (instructorAuth.isAdminEmail(user.email)) return true;
